...
import com.amazon.ask.model.interfaces.alexa.presentation.html.MessageRequest;
...
public interface MessageRequestHandler extends RequestHandler {
     ...
     */
    boolean canHandle(HandlerInput input, MessageRequest messageRequest);
     ...
     */
    Optional<Response> handle(HandlerInput input, MessageRequest messageRequest);

    @Override
    default boolean canHandle(HandlerInput handlerInput) {
        if (handlerInput.getRequest() instanceof MessageRequest) {
            return canHandle(handlerInput, (MessageRequest)handlerInput.getRequest());
        }
        return false;
    }
    ...
    default Optional<Response> handle(HandlerInput handlerInput) {
        return handle(handlerInput, (MessageRequest)handlerInput.getRequest());
    }
